...
- [ ] **L-02 Query/continuity** — search, sort, media-type, tag, rating, review, size, creator, source and group filters; previous/next; preserve query, scroll and selection. Status: partial. Native Library keeps one Slint media model across selection updates, binds scroll position to window state across workspace switches, and clears stale cursors and paging affordances on source/group navigation (unit tested). Missing filters, per-query scroll behavior, and Windows/Linux manual results remain open.
